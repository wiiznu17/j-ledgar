    ...
    <div className="min-h-screen bg-white text-foreground selection:bg-primary selection:text-white overflow-hidden">
      ...
      <nav className="relative z-10 max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-muted-foreground">
          ...
          <Link href="/login" className="px-5 py-2.5 rounded-full border border-border hover:bg-secondary transition-colors text-foreground">
            Sign In
          </Link>
        </div>
      </nav>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-32 flex flex-col items-center text-center">
        ...
        <p className="text-xl text-muted-foreground max-w-2xl mb-12 leading-relaxed">
          The ultimate engine for high-frequency financial operations. 
          Built with double-entry integrity, cloud-native scalability, and developer-first simplicity.
        </p>

        <div className="flex flex-col sm:flex-row items-center gap-4">
          <Link href="/login">
            <Button size="lg" className="h-14 px-8 text-lg font-bold rounded-full bg-gradient-to-r from-[var(--color-magenta)] to-[var(--color-pink)] hover:opacity-90 shadow-xl shadow-magenta/20 border-0 text-white">
              Launch Admin Portal
              <ArrowRight className="ml-2 w-5 h-5" />
            </Button>
          </Link>
          <Button variant="ghost" size="lg" className="h-14 px-8 text-lg font-medium rounded-full text-foreground hover:bg-secondary">
            View Documentation
          </Button>
        </div>
      </section>
      ...
      {/* Features Grid */}
      <section id="features" className="relative z-10 max-w-7xl mx-auto px-6 py-32 border-t border-border/50">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-[#2D3748]">
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground leading-relaxed">
              Every transaction is verified with immutable double-entry records, ensuring zero discrepancy across your entire system.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground leading-relaxed">
              Engineered with Spring Boot, Kafka, and Redis to handle millions of transactions per second with low latency.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground leading-relaxed">
              Comprehensive admin dashboard with live charts, reconciliation reports, and deep dives into every ledger entry.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="architecture" className="relative z-10 bg-[#FAF5FF]/30 border-y border-border py-32 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-8 text-[#2D3748]">
              Engineered for <br/> 
              <span className="text-accent underline decoration-[#BF3FFF]/30 underline-offset-8">Reliability</span>
            </h2>
          ...
          <div className="relative">
             ...
             <div className="relative bg-white border border-border rounded-3xl p-8 shadow-2xl">
                <div className="flex items-center gap-4 mb-8">
                   ...
                   <div className="text-[10px] font-mono text-muted-foreground bg-secondary px-3 py-1 rounded-full uppercase tracking-wider">core-service-v1.log</div>
                </div>
                <div className="font-mono text-sm space-y-2 text-muted-foreground">
                   <p className="text-accent">{"["}INFO{"]"} Initializing transaction context...</p>
                   <p className="text-emerald-600">{"["}SUCCESS{"]"} Account 1421 balance updated.</p>
                   <p className="text-[var(--color-magenta)]">{"["}EVENT{"]"} Kafka producer published: TX_COMMITTED</p>
                   <p className="text-muted-foreground opacity-50">{"["}INFO{"]"} Ledger sweep completed in 12ms.</p>
                   <p className="text-emerald-600">{"["}SUCCESS{"]"} Reconciliation status: PERFECT_MATCH</p>
                </div>
             </div>
          </div>
        </div>

  ...
  const renderMethod = (id: string, icon: any, title: string, subtitle: string) => (
    ...
    >
      <View className="flex-row items-center gap-5">
        <View className={`w-14 h-14 rounded-2xl items-center justify-center ${selectedMethod === id ? 'bg-secondary/10' : 'bg-[#eff0f7]'}`}>
          {React.cloneElement(icon, { size: 24, color: selectedMethod === id ? "#f48fb1" : "#4855a5" })}
        </View>
        <View>
          <Text className="text-base font-manrope font-black text-on-surface">{title}</Text>
          <Text className="text-[10px] font-manrope font-bold text-on-surfaceVariant/60 uppercase tracking-widest mt-1">{subtitle}</Text>
        </View>
      </View>
      <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${selectedMethod === id ? 'bg-secondary border-secondary' : 'border-outline-variant/20'}`}>
    ...
    <SafeAreaView className="flex-1 bg-[#f5f6fc]">
      <View className="flex-1 px-6">
        <View className="flex-row items-center justify-between mt-6 mb-10">
          ...
          <Text className="text-xl font-manrope font-black text-on-surface tracking-tight">Top Up Wallet</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <GlassPanel intensity={40} className="p-8 mb-10 items-center overflow-hidden border-secondary/5">
             ...
             <Text className="text-[10px] font-manrope font-black text-on-surfaceVariant/60 uppercase tracking-[0.2em] mb-10">Select Top Up Amount</Text>
          ...
          {/* Methods Section */}
          <Text className="text-[10px] font-manrope font-black text-secondary uppercase tracking-[0.3em] mb-5 px-1">Payment Method</Text>

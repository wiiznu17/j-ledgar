...
export const TransactionRecipientCard: React.FC<TransactionRecipientCardProps> = ({
  name,
  subtitle,
  type,
  onClear,
    ...
    <View className="bg-white rounded-[2.5rem] p-6 border border-gray-100 flex-row items-center shadow-sm">
      <View 
        className={`w-16 h-16 rounded-[1.5rem] items-center justify-center border ${
          type === 'merchant' ? 'bg-pink-50 border-pink-100' : 'bg-gray-50 border-gray-100'
        }`}
      >
        {type === 'merchant' ? (
      ...
      <View className="ml-4 flex-1">
        <View className="flex-row justify-between items-start">
          <View className="flex-1">
            ...
            {subtitle && (
              <Text className="text-xs font-manrope font-bold text-gray-400" numberOfLines={1}>
                {subtitle}
              </Text>
            )}
          </View>
          ...
          {onClear && (
            ...
            >
              <Text className="text-[10px] font-manrope font-black text-gray-400 uppercase">Change</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>

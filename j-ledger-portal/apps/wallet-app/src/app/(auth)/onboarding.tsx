        ...
        <View className="flex-1 px-6">
          ...
          >
            ...
            <StepWrapper visible={step === 'WELCOME'} direction="vertical">
              <View className="items-center">
                ...
                <View className="w-full space-y-4">
                  ...
                  >
                    <Text className="text-on-surfaceVariant font-manrope font-bold">Already have an account? <Text className="text-primary">Sign In</Text></Text>
                  </TouchableOpacity>
                </View>
              </View>
            </StepWrapper>
            ...
            <StepWrapper visible={step === 'PHONE_INPUT'}>
              {renderHeader("Enter Phone", "First, tell us your mobile number to receive a verification code.", () => setStep('WELCOME'))}
              ...
              <AppButton 
                ...
                onPress={() => { setStep('OTP'); setTimer(60); }}
                icon={<ArrowRight size={20} color="white" />}
              />
            ...
            <StepWrapper visible={step === 'OTP'}>
              {renderHeader("Verification", `We've sent a 6-digit code to ${formatPhone(phone)}`, () => setStep('PHONE_INPUT'))}
              
              <View className="flex-row justify-between mb-10">
                {otp.map((digit, i) => (
                  <View key={i} className="w-[14%] aspect-[0.75] bg-white/40 border border-outline-variant/20 rounded-2xl items-center justify-center shadow-inner overflow-hidden">
                    <AppTextInput 
              ...
              >
                ...
                <Text className={`font-manrope font-bold text-sm ${timer > 0 ? 'text-primary' : 'text-on-surfaceVariant'}`}>
            ...
            <StepWrapper visible={step === 'TERMS'}>
              {renderHeader("Terms of Service", "Please review and accept our terms before continuing.", () => setStep('OTP'))}
              
              <GlassPanel className="h-[350] mb-8" intensity={20}>
                <ScrollView showsVerticalScrollIndicator={false}>
                  <View className="space-y-6">
                    <View>
                      <Text className="font-manrope font-black text-on-surface uppercase mb-3 text-[10px] tracking-widest">1. Introduction</Text>
                      <Text className="font-manrope font-medium text-on-surfaceVariant text-xs leading-relaxed">
                        Welcome to J-Ledger. Our platform provides advanced financial services with a focus on security and transparency. By creating an account, you agree to comply with our Terms of Service and Privacy Policy.
                      </Text>
                    </View>
                    <View>
                      <Text className="font-manrope font-black text-on-surface uppercase mb-3 text-[10px] tracking-widest">2. User Eligibility</Text>
                      <Text className="font-manrope font-medium text-on-surfaceVariant text-xs leading-relaxed">
                        You must be at least 15 years old and provide a valid national ID card for E-KYC verification. Identity theft or falsification will results in permanent account suspension.
                      </Text>
                    </View>
                    <View>
                      <Text className="font-manrope font-black text-on-surface uppercase mb-3 text-[10px] tracking-widest">3. Account Security</Text>
                      <Text className="font-manrope font-medium text-on-surfaceVariant text-xs leading-relaxed">
                        You are responsible for maintaining the confidentiality of your PIN and password. J-Ledger is not liable for unauthorized access resulting from user negligence.
                      </Text>
                    </View>
                    <View>
                      <Text className="font-manrope font-black text-on-surface uppercase mb-3 text-[10px] tracking-widest">4. Financial Regulations</Text>
                      <Text className="font-manrope font-medium text-on-surfaceVariant text-xs leading-relaxed">
            ...
            <StepWrapper visible={step === 'OCR_GUIDE'} direction="vertical">
              <View className="items-center text-center">
                ...
                {renderHeader("ID Verification", "We need to scan your National ID Card. Please ensure the card is well-lit and fits within the frame.")}
                <AppButton 
            ...
            <StepWrapper visible={step === 'OCR_SCAN'}>
              <View className="items-center">
                ...
                >
                   {!isCapturing ? (
                     <View className="items-center">
                        ...
                        <Text className="text-xs font-manrope font-bold text-on-surfaceVariant mt-4 uppercase tracking-widest">Tap to start scan</Text>
                     </View>
                   ) : (
                     <View className="w-full px-12 items-center">
                        ...
                        <Text className="text-sm font-manrope font-black uppercase tracking-[0.2em] animate-pulse">Scanning...</Text>
                     </View>
                   )}
                ...
                <Text className="text-xs font-manrope font-bold text-on-surfaceVariant/60 mt-10">Ensure the front of your ID card is clear</Text>
              </View>
            </StepWrapper>

            {/* STEP 6: OCR_REVIEW */}
            <StepWrapper visible={step === 'OCR_REVIEW'}>
              {renderHeader("Confirm Details", "Please check if your ID information was scanned correctly.")}
              ...
              <View className="aspect-[1.58] w-full bg-white/40 rounded-3xl overflow-hidden shadow-inner mb-10 border border-outline-variant/10">
                ...
                <View className="absolute bottom-0 left-0 right-0 bg-black/60 p-6 backdrop-blur-md">
                   <Text className="text-[10px] font-manrope font-bold uppercase tracking-[0.2em] text-white/70 mb-1">National ID - Thai</Text>
            ...
            <StepWrapper visible={step === 'FACE_GUIDE'} direction="vertical">
              <View className="items-center text-center">
                ...
                {renderHeader("Biometric Scan", "Last step of identity check. We'll capture a short selfie to secure your account.")}
                <AppButton 
                ...
                >
                  {!isCapturing ? (
                    ...
                    <View className="items-center">
                      ...
                      />
                      <Text className="text-4xl font-manrope font-black text-on-surface">{captureProgress}%</Text>
                      <Text className="text-[10px] font-manrope font-black uppercase tracking-[0.3em] animate-pulse mt-4 text-primary">Capturing...</Text>
                    </View>
                  )}
                </TouchableOpacity>
                <Text className="text-xs font-manrope font-bold text-on-surfaceVariant/60 mt-12">Position your face within the circle</Text>
              </View>
            </StepWrapper>

            {/* STEP 9: ADDITIONAL_INFO */}
            <StepWrapper visible={step === 'ADDITIONAL_INFO'}>
              {renderHeader("Additional Details", "We need a bit more info to finish setting up your account.")}
              
              <View className="space-y-5">
                <View className="flex-row gap-4">
                  <AppTextInput label="First Name" value={firstName} onChangeText={setFirstName} containerClassName="flex-1" />
                  <AppTextInput label="Last Name" value={lastName} onChangeText={setLastName} containerClassName="flex-1" />
                </View>
                <AppTextInput label="Home Address" value={address} onChangeText={setAddress} multiline placeholder="Full address..." />
                
                <View className="flex-row gap-4">
                  <AppTextInput label="Occupation" value={occupation} onChangeText={setOccupation} containerClassName="flex-1" placeholder="e.g. Employee" />
                  <AppTextInput label="Income" value={income} onChangeText={setIncome} containerClassName="flex-1" placeholder="Monthly" />
                </View>
                
                <View className="flex-row gap-4">
                  <AppTextInput label="Source of funds" value={sourceOfFunds} onChangeText={setSourceOfFunds} containerClassName="flex-1" placeholder="e.g. Salary" />
                  <AppTextInput label="Purpose" value={purpose} onChangeText={setPurpose} containerClassName="flex-1" placeholder="e.g. Savings" />
                </View>
            ...
            <StepWrapper visible={step === 'SET_PASSWORD'}>
              <View className="items-center mb-10">
                ...
                {renderHeader("Account Security", "Create a secure password to protect your transactions.")}
              </View>
            ...
            <StepWrapper visible={step === 'SUCCESS'}>
              <View className="items-center">
                ...
                <GlassPanel className="w-full mb-12 shadow-2xl shadow-primary/5" intensity={25}>
                   <Text className="text-on-surface font-manrope font-extrabold text-xl text-center mb-3">Verification in Progress</Text>
                   <Text className="text-on-surfaceVariant font-manrope font-medium text-sm text-center leading-relaxed">
                     We're reviewing your documents. You'll receive a notification within <Text className="text-primary font-black">24 hours</Text> when your account is active.
                   </Text>
                </GlassPanel>

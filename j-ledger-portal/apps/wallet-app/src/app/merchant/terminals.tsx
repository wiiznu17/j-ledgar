...
import { ChevronLeft, CreditCard, MonitorSmartphone } from 'lucide-react-native';
      ...
      <View className="bg-white p-5 rounded-[1.5rem] mb-4 border border-gray-100 shadow-sm">
        <View className="flex-row items-center justify-between mb-3">
          <View className="flex-row items-center">
            <View className={`w-12 h-12 rounded-2xl items-center justify-center mr-4 ${
              ...
              <MonitorSmartphone size={24} color={isActive ? '#8b5cf6' : '#9ca3af'} />
            </View>
            <View>
        ...
        <View className="bg-gray-50 p-3 rounded-xl border border-dashed border-gray-200 mt-2">
          ...
          <View className="mt-2 flex-row justify-between px-2">
            <Text className="text-xs text-gray-500 font-medium">Created On:</Text>
            <Text className="text-xs text-gray-800 font-bold">{new Date(item.createdAt).toLocaleDateString()}</Text>
          </View>
          <View className="mt-2 flex-row justify-between px-2">
            <Text className="text-xs text-gray-500 font-medium">Authentication:</Text>
    ...
    <SafeAreaView className="flex-1 bg-[#f8f9fe]" edges={['top']}>
      ...
      <View className="flex-row items-center justify-between px-5 pt-2 pb-6">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-black font-manrope text-gray-800">My Terminals</Text>
        </View>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="items-center justify-center py-20">
              ...
              <Text className="font-manrope font-bold text-gray-400 text-base">No terminals configured.</Text>
            </View>
          }
        />
      )}

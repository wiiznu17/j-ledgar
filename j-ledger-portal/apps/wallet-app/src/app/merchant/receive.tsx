...
export default function MerchantPaymentQRScreen() {
  ...
  const handleGenerateDynamicQR = async () => {
    if (!amount || parseFloat(amount || '0') < 5.0) {
    ...
    try {
      const result = await MerchantService.generatePaymentQR(
        merchantId,
        parseFloat(amount),
      );
      setQrData(result);
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {merchantProfile?.category || 'Retailer'}
            </Text>

            <View
              style={{
                backgroundColor: amount ? '#fdf2f8' : '#f0fdf4',
                paddingHorizontal: 12,
                paddingVertical: 4,
                borderRadius: 100,
                marginBottom: 16,
                borderWidth: 1,
                borderColor: amount ? '#fbcfe8' : '#dcfce7',
              }}
            >
              <Text
                style={{
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderRadius: 14,
                    backgroundColor:
                      isGenerating || !amount ? '#fbcfe8' : '#f48fb1',
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
                >
          ...
          <View style={{ flexDirection: 'row', gap: 12, marginTop: 16 }}>
            ...
            <TouchableOpacity
              onPress={() =>
                ...
                shareQR(
                  qrData.qrCode,
                  `Pay ฿${amount || 'any amount'} to ${merchantProfile?.name}\n${qrData?.payUrl}`,
                )
              }
              style={{
                flex: 1,

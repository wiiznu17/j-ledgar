...
import { View, Text, TextInput, TouchableOpacity, ScrollView, Alert, ActivityIndicator, Image, KeyboardAvoidingView, Platform, Keyboard, Dimensions, Modal, FlatList } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { ChevronLeft, Store, FileText, CheckCircle2, User, Mail, Phone, MapPin, Tag, Camera, X, ShieldCheck, Search, Navigation2, Plus, Image as ImageIcon, AlertCircle, Maximize2, Map as MapIcon, LocateFixed, Clock } from 'lucide-react-native';
...
const ProgressBar = ({ step }: { step: number }) => (
  <View className="px-6 pt-2 mb-6">
    <View className="flex-row justify-between items-center mb-3">
      <Text className="text-indigo-600 font-black font-manrope text-[10px] uppercase tracking-widest">Step {step} of 3</Text>
      <Text className="text-slate-400 font-black text-[10px]">{Math.round((step / 3) * 100)}% Complete</Text>
    </View>
...
const InfoRow = ({ label, value }: { label: string, value: string }) => (
  <View className="flex-row justify-between py-2 border-b border-slate-50">
    ...
    <Text className="text-slate-700 text-[11px] font-black">{value || '-'}</Text>
  </View>
);

const InputField = ({ label, icon: Icon, value, onChange, placeholder, keyboardType = 'default', multiline = false, error = '', editable = true }: any) => (
  <View className="mb-5">
    <View className="flex-row items-center mb-2 ml-1">
      <Icon size={12} color={error ? "#f43f5e" : "#6366f1"} />
      <Text className={`text-[10px] font-black uppercase tracking-widest ml-2 ${error ? 'text-rose-500' : 'text-indigo-500'}`}>{label}</Text>
    </View>
    <TextInput
      value={value}
    ...
    {error ? (
      <View className="flex-row items-center mt-1.5 ml-2">
        ...
        <Text className="text-[9px] font-bold text-rose-500 ml-1 italic">{error}</Text>
      </View>
    ) : null}
  </View>
);
...
export default function MerchantApply() {
  ...
  const [tempCoords, setTempCoords] = useState({ latitude: 13.7563, longitude: 100.5018 });
        ...
        if (results && results.length > 0) {
          ...
          const suggestedItems = await Promise.all(
            results.slice(0, 3).map(async (res) => {
              ...
              return {
                ...res,
                title: [r?.name, r?.street, r?.district, r?.region].filter(Boolean).join(', ') || text
              };
            })
          );
  ...
  const confirmLocation = () => {
    setForm(prev => ({ ...prev, latitude: tempCoords.latitude, longitude: tempCoords.longitude }));
  ...
  const handleChange = (field: string, value: string) => {
    ...
    if (field === 'businessName') {
      ...
      if (value && !thaiRegex.test(value)) {
        setErrors(prev => ({ ...prev, businessName: 'Please use Thai characters only.' }));
      } else {
    ...
    if (field === 'businessNameEn') {
      ...
      if (value && !engRegex.test(value)) {
        setErrors(prev => ({ ...prev, businessNameEn: 'Please use English characters only.' }));
      } else {
  ...
  const nextStep = () => {
    if (step === 1) {
      if (!form.businessName || !form.category) {
        Alert.alert('Required Fields', 'Please fill in business name and category.');
        return;
      }
      ...
      if (form.phone.length < 9 || form.phone.length > 10) {
        Alert.alert('Invalid Phone', 'Please enter a valid 9 or 10-digit phone number.');
        return;
      }
    }
  ...
  const handleApply = async () => {
    ...
    if (form.images.length < 3) {
      Alert.alert('Photos Required', 'Please upload at least 3 photos of your storefront.');
      return;
    }
    ...
    try {
      ...
      const uploadRes = await api.post('/merchant/upload-storefront-images', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      const s3Urls = (uploadRes.data.urls || []).map((img: any) => typeof img === 'string' ? img : img.url);
      ...
      console.log('[Merchant Apply] Submitting payload:', JSON.stringify(payload, null, 2));
  ...
  if (isSuccess) {
    return (
      <SafeAreaView className="flex-1 bg-white items-center justify-center px-6">
        ...
        <Text className="text-3xl font-black font-manrope text-gray-800 text-center mb-3">Success!</Text>
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#f8f9fe]" edges={['top']}>
          ...
          <SafeAreaView edges={['top']} className="bg-white">
            <View className="z-20">
              <View className="flex-row items-center px-5 pt-4 pb-2">
                <TouchableOpacity onPress={() => { setShowMapModal(false); setSuggestions([]); }} className="p-2">
              ...
              <View className="flex-1 flex-row items-center bg-slate-100 px-5 py-3.5 rounded-xl ml-2 border border-slate-200">
                ...
                {isSearchingMap && <ActivityIndicator size="small" color="#6366f1" className="ml-2" />}
              </View>
            </View>
                  ...
                  {suggestions.map((item, index) => (
                    ...
                    >
                      ...
                      <View className="flex-1">
                        <Text className="text-gray-800 font-bold text-xs" numberOfLines={1}>{item.title}</Text>
                        <Text className="text-slate-400 text-[9px] mt-1">Lat: {item.latitude.toFixed(4)}, Lng: {item.longitude.toFixed(4)}</Text>
                      </View>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
        ...
        <View className="flex-1 relative z-10">
            ...
            <View className="absolute bottom-10 left-6 right-6">
              <TouchableOpacity 
                ...
                className="bg-indigo-600 py-4 rounded-[2rem] shadow-2xl shadow-indigo-200 flex-row items-center justify-center"
              >
                <LocateFixed size={20} color="white" />
                <Text className="text-white font-black font-manrope text-lg ml-3">Confirm Pin Location</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <View className="flex-row items-center justify-between px-5 pt-2 pb-2">
        <TouchableOpacity onPress={() => step > 1 ? prevStep() : router.back()} className="p-2 bg-white rounded-xl shadow-sm border border-slate-50">
          <ChevronLeft size={20} color="#1f2937" />
        </TouchableOpacity>
        <Text className="text-lg font-black font-manrope text-gray-800">Merchant Registration</Text>
      ...
      >
        ...
        {step === 1 && (
          <View>
            <View className="mb-8">
              <Text className="text-2xl font-black font-manrope text-gray-800 mb-2">Business Details</Text>
              <Text className="text-slate-400 font-bold text-sm leading-relaxed">Complete the form below to apply for a merchant partnership.</Text>
            </View>

            <View className="bg-white rounded-3xl p-6 mb-8 shadow-sm border border-slate-100">
              <View className="flex-row items-center mb-4">
                <ShieldCheck size={16} color="#6366f1" />
                <Text className="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-2">Owner Identity (KYC)</Text>
              </View>
              {isLoadingUser ? (
                ...
                <View>
                  ...
                  <InfoRow label="ID Card" value={userData?.idCardNumber || '1-3097-****-*-**-*'} />
                  <InfoRow label="Birth Date" value={userData?.birthDate ? new Date(userData.birthDate).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'} />
                  <InfoRow label="Phone" value={userData?.phone || '***-***-****'} />
                </View>
              )}
            </View>

            <Text className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] mb-4 ml-1">Section 1: Store Profile</Text>
        ...
        {step === 2 && (
          <View>
            <View className="mb-8">
              <Text className="text-2xl font-black font-manrope text-gray-800 mb-2">Agreement</Text>
              <Text className="text-slate-400 font-bold text-sm leading-relaxed">Review our terms of service for merchant partners.</Text>
            </View>
            ...
            <View className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm h-96">
              <ScrollView showsVerticalScrollIndicator={false}>
                <Text className="text-slate-500 font-bold leading-6 text-sm">
                  1. Service Usage: The merchant agrees to use the service solely for receiving payments for legitimate goods and services...
                  {"\n\n"}
                  2. Fees: Transaction fees (MDR) will be applied as specified in the merchant contract...
                  {"\n\n"}
                  3. Security: Merchants must maintain strict confidentiality of terminal keys and access credentials...
                  {"\n\n"}
                  4. Review Period: Verification typically takes 1-3 business days...
                </Text>
              </ScrollView>
            </View>
            ...
            >
              <View className={`w-6 h-6 rounded-lg items-center justify-center border ${acceptedTerms ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-slate-200'}`}>
                {acceptedTerms && <CheckCircle2 size={14} color="white" />}
              </View>
              <Text className="ml-3 text-slate-700 font-black font-manrope text-xs">I accept the terms and conditions</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {step === 3 && (
          <View>
            <View className="mb-8">
              <Text className="text-2xl font-black font-manrope text-gray-800 mb-2">Store Location & Photos</Text>
              <Text className="text-slate-400 font-bold text-sm leading-relaxed">Pin your store location and upload some photos.</Text>
            </View>
            ...
            <View className="mb-8">
              <TouchableOpacity 
                onPress={() => {
                  setTempCoords({ latitude: form.latitude, longitude: form.longitude });
              ...
              >
                ...
                <View className="absolute inset-0 bg-black/5 items-center justify-center">
                   <View className="bg-white/90 px-4 py-2 rounded-full flex-row items-center shadow-sm">
                      <MapIcon size={14} color="#6366f1" />
                      <Text className="text-indigo-600 font-black text-[10px] ml-2">TAP TO PIN LOCATION</Text>
                   </View>
                </View>
                {isReverseGeocoding && (
            ...
            <View className="mb-4">
              <View className="flex-row items-center justify-between mb-4">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-2">Store Photos</Text>
                </View>
                <Text className={`text-[10px] font-black ${form.images.length >= 3 ? 'text-emerald-500' : 'text-slate-400'}`}>
              ...
              <View className="flex-row flex-wrap">
                {form.images.map((uri, index) => (
                  <View key={index} className="w-[31%] aspect-square mr-[3%] mb-3 relative">
                ...
                {form.images.length < 5 && (
                  ...
                  >
                    ...
                    <Text className="text-indigo-400 text-[8px] font-black uppercase mt-1">Add Photo</Text>
                  </TouchableOpacity>
                )}
              </View>
              ...
              {form.images.length < 3 && (
                <View className="flex-row items-center mt-1 ml-1">
                  ...
                  <Text className="text-[9px] font-bold text-slate-400 ml-1.5 italic">Please upload at least 3 photos</Text>
                </View>
              )}
            </View>

            <View className="bg-indigo-50/50 p-5 rounded-2xl border border-indigo-100/50 mt-4 flex-row items-start">
              <FileText size={16} color="#6366f1" className="mt-0.5" />
              <Text className="text-indigo-600 text-[11px] font-bold leading-5 ml-3 flex-1">
                Tip: Clear photos and accurate location help speed up the approval process.
              </Text>
            </View>
          </View>
        )}
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white/80 p-6 border-t border-slate-50">
        <TouchableOpacity 
          ...
          disabled={isSubmitting || (step === 2 && !acceptedTerms) || (step === 3 && (form.images.length < 3 || !form.address))}
        ...
        {step > 1 && (
          <TouchableOpacity onPress={prevStep} className="mt-4">
            <Text className="text-center text-slate-400 font-black font-manrope text-xs uppercase tracking-widest">Go Back</Text>
          </TouchableOpacity>
        )}
      </View>
    </SafeAreaView>

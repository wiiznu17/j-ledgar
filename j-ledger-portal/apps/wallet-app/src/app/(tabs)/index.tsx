...
const RECENT_TRANSACTIONS = [
  { id: '1', title: 'Gourmet Groceries', category: 'Shopping', amount: 1240, type: 'expense', time: '10:24 AM' },
  { id: '2', title: 'Salary Deposit', category: 'Deposit', amount: 45000, type: 'income', time: 'Yesterday' },
];

export default function HomeScreen() {
  const router = useRouter();

  const renderServiceItem = (icon: any, label: string, color: string, route?: string, comingSoon?: boolean) => (
    <TouchableOpacity 
      ...
      >
        ...
        {comingSoon && (
          <View className="absolute -top-1 -right-1 bg-on-surfaceVariant px-1 py-0.5 rounded shadow-sm">
            <Text className="text-[6px] font-black text-white uppercase tracking-tighter">Soon</Text>
          </View>
        )}
      </MotiView>
      <Text className="text-[10px] font-manrope font-bold text-on-surfaceVariant text-center tracking-tight">{label}</Text>
    </TouchableOpacity>
  );
      ...
      >
        ...
        >
          <View className="flex-row items-center gap-3">
            <Image source={MOCK_USER.avatar} className="w-12 h-12 rounded-full border-2 border-primary" />
            <View>
              <Text className="text-xs font-manrope font-medium text-on-surfaceVariant">Hello,</Text>
              <Text className="text-base font-manrope font-black text-on-surface">{MOCK_USER.name}</Text>
            </View>
          </View>
        </MotiView>
        ...
        <View className="space-y-4 mb-8">
              ...
              <View className="space-y-6">
                <View className="flex-row justify-between items-center px-2">
                  <View className="flex-row items-center gap-2">
                    ...
                    <Text className="text-[10px] font-manrope font-black uppercase tracking-[0.2em] text-on-surfaceVariant opacity-60">Active Balance</Text>
                  </View>
                  <TouchableOpacity onPress={() => router.push('/(tabs)/history' as any)} className="flex-row items-center gap-1">
                    <Text className="text-[10px] font-manrope font-black text-primary uppercase">History</Text>
           ...
           >
              <View className="flex-row items-center gap-4">
                 ...
                 <View>
                    <Text className="text-[10px] font-manrope font-black uppercase tracking-[0.2em] text-on-surfaceVariant opacity-40">Reward Points</Text>
                    <Text className="text-xl font-manrope font-black text-on-surface">1,250 <Text className="text-sm text-on-surfaceVariant">pts</Text></Text>
                 </View>
              </View>
              <TouchableOpacity className="bg-surface-container-low px-4 py-2 rounded-lg items-center justify-center">
                 <Text className="text-[10px] font-manrope font-black text-primary uppercase tracking-widest">Redeem</Text>
              </TouchableOpacity>
           </MotiView>
        </View>
        ...
        <View className="mb-10">
           <ScrollView horizontal showsHorizontalScrollIndicator={false} className="-mx-5 px-5">
              <TouchableOpacity className="w-[300] aspect-[21/9] rounded-[1.8rem] overflow-hidden mr-4 shadow-xl shadow-black/5 relative group">
                 <Image source={{ uri: 'https://picsum.photos/seed/promo1/600/300' }} className="w-full h-full" resizeMode="cover" />
                 <View className="absolute inset-0 bg-black/40 p-5 justify-center">
                   <View className="bg-primary self-start px-2 py-0.5 rounded mb-2">
                     <Text className="text-white text-[8px] font-black tracking-widest uppercase">Hot Deals</Text>
                   </View>
                   <Text className="text-white font-manrope font-black text-lg leading-tight">Get ฿50 Cashback</Text>
                   <Text className="text-white font-manrope font-medium text-xs">on your first Bill Pay</Text>
                 </View>
              </TouchableOpacity>
              <TouchableOpacity className="w-[300] aspect-[21/9] rounded-[1.8rem] overflow-hidden mr-10 shadow-xl shadow-black/5 relative">
                 <Image source={{ uri: 'https://picsum.photos/seed/promo2/600/300' }} className="w-full h-full" resizeMode="cover" />
                 <View className="absolute inset-0 bg-black/40 p-5 justify-center">
                   ...
                   <Text className="text-white font-manrope font-black text-lg leading-tight">Double Points</Text>
                   <Text className="text-white font-manrope font-medium text-xs">every Friday with Scan & Pay</Text>
                 </View>
              </TouchableOpacity>
           </ScrollView>
        </View>
                ...
                >
                  <View className="flex-row items-center gap-4">
                    <View className="w-12 h-12 rounded-full bg-surface-container flex-row items-center justify-center">
                       {tx.category === 'Shopping' ? <ShoppingBag size={22} color="#f48fb1" /> : <Landmark size={22} color="#f48fb1" />}
                    </View>
                    <View>
                      <Text className="font-manrope font-black text-on-surface text-sm">{tx.title}</Text>
                      <Text className="font-manrope text-[10px] font-extrabold text-on-surfaceVariant mt-0.5 uppercase tracking-widest">{tx.category}</Text>
                    </View>
                  </View>
                  <Text className={`font-manrope font-black text-base ${tx.type === 'income' ? 'text-green-600' : 'text-on-surface'}`}>
                    {tx.type === 'expense' ? '-' : '+'}{MOCK_USER.currency}{tx.amount.toLocaleString()}
                  </Text>
                </TouchableOpacity>
              ))}
           </View>

        ...
        >
          {CATEGORIES.map((cat) => (
            ...
            >
              ...
              >
                <Text
                  className={`font-manrope font-black text-[10px] uppercase tracking-widest ${selectedCategory === cat ? 'text-white' : 'text-on-surface'}`}
                >
                  {cat}
                </Text>
              </MotiView>
      ...
      >
        <View className="space-y-4">
          {filteredTransactions.map((tx, idx) => (
            ...
            >
              <TouchableOpacity className="bg-white/40 border border-outline-variant/5 rounded-[28] p-5 flex-row items-center justify-between shadow-sm active:scale-95">
                <View className="flex-row items-center gap-5">
                  <View className="w-14 h-14 rounded-2xl bg-[#eff0f7] items-center justify-center relative">
                    ...
                    <View
                      className={`absolute bottom-[-2] right-[-2] w-6 h-6 rounded-full items-center justify-center border-2 border-white ${tx.type === 'income' ? 'bg-green-500' : 'bg-red-500'}`}
                    >
                      {tx.type === 'income' ? (
                ...
                <View className="items-end">
                  ...
                  <Text className="text-[9px] font-manrope font-black text-on-surfaceVariant/30 uppercase tracking-tighter mt-1">
                    Success
                  </Text>
                </View>
              </TouchableOpacity>

...
export function RedemptionsTable({ redemptions }: RedemptionsTableProps) {
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'REDEEMED':
        return <Badge className="bg-blue-50 text-blue-600 border-blue-100">Ready</Badge>;
      case 'USED':
        return <Badge className="bg-green-50 text-green-600 border-green-100">Used</Badge>;
      case 'EXPIRED':
        return <Badge className="bg-red-50 text-red-600 border-red-100">Expired</Badge>;
      default:
          ...
          {redemptions.map((item) => (
            <TableRow key={item.id} className="hover:bg-secondary/30 transition-colors">
              ...
              <TableCell>
                ...
                <div className="text-[10px] text-muted-foreground">{item.deal?.brand?.name}</div>
              </TableCell>
              <TableCell>
              ...
              <TableCell>
                <div className="space-y-1 text-[10px] text-muted-foreground">
                  <div className="flex items-center gap-1">
                    <Calendar size={10} /> Redeemed: {new Date(item.createdAt).toLocaleDateString()}
                  </div>
                  {item.status === 'USED' && (
                    <div className="flex items-center gap-1 text-green-500 font-medium">
                      <CheckCircle2 size={10} /> Used: {new Date(item.usedAt).toLocaleDateString()}
                    </div>
                  )}
                  {item.status === 'REDEEMED' && (
                    <div className="flex items-center gap-1 text-orange-500">
                      <Clock size={10} /> Expires: {new Date(item.expiresAt).toLocaleDateString()}
                    </div>
                  )}
                </div>
              </TableCell>

...
const CharCounter = ({ current, max }: { current: number, max: number }) => (
  <div className={`text-[9px] font-bold text-right mt-1 ${current > max ? 'text-destructive' : 'text-muted-foreground'}`}>
...
export function DealForm({ initialData, onSuccess, onCancel, isPage = false }: DealFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-6 text-foreground bg-card">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6 flex flex-col h-full">
            <div className="bg-card p-5 rounded-[1.5rem] border border-border shadow-xs flex-1">
                <SectionHeader icon={ImageIcon} title="Deal Media" colorClass="bg-indigo-500/10 text-indigo-600 dark:text-indigo-400" />
                <ImageUploadWithCrop
                    label="Cover Banner"
            ...
            <div className="bg-card p-5 rounded-[1.5rem] border border-border shadow-xs">
                <SectionHeader icon={ShoppingBag} title="Logistics & Rules" colorClass="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" />
                <div className="grid grid-cols-3 gap-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="points" className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Points</Label>
                        <div className="relative">
                            ...
                            />
                            <Tag size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-indigo-600 dark:text-indigo-400" />
                        </div>
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="stock" className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Stock</Label>
                        <Input
                            id="stock"
                            type="text"
                            inputMode="numeric"
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="limit" className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Limit Per User</Label>
                        <Input
                            id="limit"
                            type="text"
                            inputMode="numeric"
        ...
        <div className="lg:col-span-5 space-y-6 flex flex-col h-full">
            <div className="bg-card p-5 rounded-[1.5rem] border border-border shadow-xs flex-1">
                <SectionHeader icon={Info} title="Promotion Content" colorClass="bg-blue-500/10 text-blue-600 dark:text-blue-400" />
                <div className="grid grid-cols-1 gap-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="title" className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Headline</Label>
                        <Input
                            id="title"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Brand</Label>
                            <Select
                            ...
                            >
                                ...
                                <SelectContent className="rounded-xl bg-card border-border">
                                    {brands.map((b) => (
                                        <SelectItem key={b.id} value={b.id} className="font-bold py-1.5 text-foreground hover:bg-muted">
                                            {b.name}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                        ...
                        <div className="space-y-1.5">
                            ...
                            >
                                ...
                                <SelectContent className="rounded-xl bg-card border-border">
                                    {categories.map((c) => (
                                        <SelectItem key={c.id} value={c.id} className="font-bold py-1.5 text-foreground hover:bg-muted">
                                            {c.name}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="description" className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Summary</Label>
                        <Input
                            id="description"
            ...
            <div className="bg-card p-5 rounded-[1.5rem] border border-border shadow-xs">
                <SectionHeader icon={Settings2} title="Availability" colorClass="bg-purple-500/10 text-purple-600 dark:text-purple-400" />
                    ...
                    <div className="flex items-center justify-between p-2 rounded-xl bg-muted border border-border">
                        ...
                        <div 
                            ...
                            className={`relative w-12 h-6 rounded-full cursor-pointer transition-all duration-300 shadow-inner ${
                                formData.isActive ? 'bg-emerald-500' : 'bg-muted border border-border'
                            }`}
                        >
                            <div className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full shadow-md transition-all duration-300 transform ${
                                ...
                                <div className={`w-1 h-1 rounded-full ${formData.isActive ? 'bg-emerald-500' : 'bg-muted-foreground/30'}`} />
                            </div>
                            <span className={`absolute inset-0 flex items-center justify-center text-[8px] font-black pointer-events-none transition-all ${
                                formData.isActive ? 'pr-5 text-white opacity-0' : 'pl-5 text-muted-foreground opacity-100'
                            }`}>
                                OFF
                            </span>
                            <span className={`absolute inset-0 flex items-center justify-center text-[8px] font-black pointer-events-none transition-all ${
                                formData.isActive ? 'pr-5 text-white opacity-100' : 'pl-5 text-muted-foreground opacity-0'
                            }`}>
                                ON
                            </span>
                        </div>
      ...
      <div className="bg-card p-5 rounded-[1.5rem] border border-border shadow-xs">
          <SectionHeader icon={Scale} title="Legal & Fine Print" colorClass="bg-muted text-muted-foreground" />

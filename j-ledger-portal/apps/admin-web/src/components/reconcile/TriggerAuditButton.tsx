    ...
    <div className="flex items-center gap-3">
      {isLoading && (
        <div className="flex flex-col items-end animate-in slide-in-from-right-2 duration-300">
          <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Auditing Ledger</span>
          <span className="text-[9px] text-slate-400 font-medium">Verifying double-entry invariants...</span>
        </div>
      )}
      <Button
        ...
        className={`rounded-2xl px-6 h-12 font-black transition-all active:scale-95 shadow-lg shadow-indigo-200/50 flex gap-2 ${
          isLoading ? 'bg-slate-100 text-slate-400 border-slate-200' : 'bg-indigo-600 hover:bg-indigo-700 text-white'
        }`}
      >
        {isLoading ? (

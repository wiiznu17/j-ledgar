...
export function TransactionVolumeChart({
  ...
  const totalTransactions = data.reduce((sum, item) => sum + (item.volume || 0), 0);
      ...
      <Card className="border border-border/80 shadow-md shadow-slate-200/40 dark:shadow-none hover:shadow-lg hover:border-indigo-500/20 transition-all duration-300 rounded-xl bg-card">
        <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
          <div className="flex items-center gap-1.5">
            <CardTitle className="text-sm font-semibold text-foreground">Transaction Volume</CardTitle>
            <InfoTooltip
        ...
        <div className="flex items-baseline gap-2.5 mb-5 mt-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="px-1.5 py-0.5 rounded-full text-[9px] font-extrabold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400">
              +15.6% <span className="font-medium text-muted-foreground text-[8px] ml-0.5">vs last 30 days</span>
            </span>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%" minWidth={0}>
            ...
            >
              ...
              <Tooltip
                ...
                itemStyle={{ color: '#1E293B', fontWeight: 600, fontSize: '11px' }}
                labelStyle={{ color: '#64748B', fontWeight: 600, fontSize: '10px', marginBottom: '2px' }}
              />
              <Area
                type="monotone"
                dataKey="volume"

...
export default function InternalLedgerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  if (loading && !account) {
    return <div className="flex items-center justify-center h-64 animate-pulse text-slate-400">Loading details...</div>;
  }
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
             <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Ledger Detail</h1>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 flex flex-row items-center justify-between">
            ...
            <Badge
              className={cn(
                ...
                account.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
              )}
            >
              {account.status}
            </Badge>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
               <div className="space-y-1">
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Internal User Mapping</p>
                 <div className="flex items-center gap-2">
                   <User className="w-3.5 h-3.5 text-slate-400" />
                   <p className="text-sm font-bold text-slate-700 truncate" title={account.userId}>{account.userId.substring(0, 8)}...</p>
                 </div>
               </div>
               <div className="space-y-1">
                 ...
                 <div className="flex items-center gap-2">
                   ...
                   <p className="text-sm font-bold text-slate-700">{account.currency}</p>
                 </div>
               </div>
               <div className="space-y-1">
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Opened Date</p>
                 <div className="flex items-center gap-2">
                   <Calendar className="w-3.5 h-3.5 text-slate-400" />
                   <p className="text-sm font-bold text-slate-700">{new Date(account.createdAt).toLocaleDateString()}</p>
                 </div>
               </div>
               <div className="space-y-1">
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Version Control</p>
                 <div className="flex items-center gap-2">
                   <Hash className="w-3.5 h-3.5 text-slate-400" />
                   <p className="text-sm font-bold text-slate-700">v{ (account as any).version || 1 }</p>
                 </div>
               </div>
            </div>
          </CardContent>
        ...
        <Card className="border-none shadow-lg shadow-indigo-100 ring-1 ring-indigo-100 bg-indigo-600 overflow-hidden">
          <CardContent className="p-6 flex flex-col justify-center h-full relative">
            ...
            <p className="text-indigo-100 text-[10px] font-bold uppercase tracking-[0.2em] mb-2">Available Balance</p>
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-indigo-200 font-bold text-sm">{account.currency}</span>
            </div>
      ...
      <Card className="border-none shadow-sm ring-1 ring-slate-100 overflow-hidden bg-white">
        <CardHeader className="bg-slate-50/30 border-b border-slate-100">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-sm font-bold text-slate-600">Ledger Entries (Statement)</CardTitle>
              <p className="text-[10px] text-slate-400 mt-1">Detailed audit trail of all transactions affecting this account.</p>
            </div>
          </div>
        </CardHeader>
            ...
            <Table>
              <TableHeader className="bg-slate-50/50">
                <TableRow className="border-slate-100 hover:bg-transparent">
                   <TableHead className="w-[180px] text-[11px] font-bold text-slate-500 uppercase tracking-wider pl-6">Timestamp</TableHead>
                   <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Transaction Ref</TableHead>
                   <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Type</TableHead>
                   <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider text-right pr-6">Amount</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading ? (
                  ...
                  entries.map((entry) => (
                    <TableRow key={entry.id} className="border-slate-50 hover:bg-slate-50/30 transition-colors group">
                       <TableCell className="pl-6 py-4">
                         <div className="flex flex-col">
                           <span className="text-xs font-bold text-slate-700">{new Date(entry.createdAt).toLocaleDateString()}</span>
                       ...
                       <TableCell>
                          <Badge className={cn(
                            ...
                            entry.entryType === 'CREDIT' ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"
                          )}>
                            {entry.entryType}
                          </Badge>
                       </TableCell>

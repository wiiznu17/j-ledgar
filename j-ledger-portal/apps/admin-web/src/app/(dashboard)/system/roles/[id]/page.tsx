  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-4 animate-pulse">
        ...
        <p className="font-bold tracking-widest text-xs uppercase">Deciphering Security Matrix...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-20">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest gap-2">
          <button onClick={() => router.push('/system/roles')} className="hover:text-indigo-600 transition-colors uppercase tracking-widest font-bold text-[10px]">Directory</button>
          <ChevronRight className="w-3 h-3" />
          <span className="text-slate-900">Configure Scope</span>
        </div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-2xl ${role.isSystem ? 'bg-indigo-600 text-white' : 'bg-emerald-600 text-white'} shadow-lg`}>
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm italic">{role.description || 'No description provided.'}</p>
            </div>
          </div>
        ...
        <div className="space-y-6 lg:col-span-1">
          <Card className="border-0 shadow-xl shadow-slate-200/50 rounded-2xl bg-white/80 backdrop-blur-md sticky top-6">
            <CardHeader className="pb-4">
              <CardTitle className="text-sm font-black text-slate-400 uppercase tracking-widest">Identity Info</CardTitle>
            </CardHeader>
              ...
              {role.isSystem && (
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex gap-3">
                  ...
                  <p className="text-[11px] text-amber-700 leading-normal font-medium">
                    This is a **Core System Role**. Identity and permissions are immutable to ensure platform integrity.
                  </p>
                </div>
              )}
            </CardContent>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex items-center justify-between mb-2 px-1">
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Authorized</span>
              </div>
              <div className="flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 gap-6">
            {Object.entries(groupedPerms).map(([resource, perms]: [string, any]) => (
              <Card key={resource} className="border-0 shadow-lg shadow-slate-100/50 rounded-2xl overflow-hidden bg-white/60 backdrop-blur-sm group">
                <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-100/50 flex items-center justify-between">
                  <h4 className="text-xs font-black text-slate-800 uppercase tracking-[0.2em]">{resource}</h4>
                  <Badge variant="outline" className="bg-white text-[10px] font-bold border-slate-200 text-slate-500">
                          ...
                          <div className="flex items-center gap-4 flex-1">
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <span className={`text-sm font-bold tracking-tight ${isSelected ? 'text-emerald-700' : 'text-slate-700'}`}>
                              ...
                              <p className="text-[11px] text-slate-400 font-medium leading-relaxed mt-0.5">
                                {perm.description || `Grants ${perm.action.toLowerCase()} access to ${resource.toLowerCase()} resources.`}
                              </p>
                            </div>
                          </div>
                          
                          <div className="ml-4">
                            <Badge className={`font-black text-[9px] px-2 py-0.5 tracking-tighter border-0 ${
                              isSelected ? 'bg-emerald-600/10 text-emerald-600' : 'bg-slate-100 text-slate-400'
                            }`}>
                              {perm.action}
                            </Badge>
                          </div>

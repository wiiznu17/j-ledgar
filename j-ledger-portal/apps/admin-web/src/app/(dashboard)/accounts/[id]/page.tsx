...
export default function WalletDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchWalletData = async (isSync = false) => {
    ...
    try {
      ...
      const filteredTx = txResponse.data.filter((tx: Transaction) => 
    ...
    <div className="space-y-4 max-w-6xl mx-auto pb-10">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest gap-2">
          <button onClick={() => router.back()} className="hover:text-indigo-600 transition-colors uppercase tracking-widest font-bold text-[10px]">Ledger Accounts</button>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {wallet.status === 'FROZEN' ? <><ShieldCheck className="w-3.5 h-3.5 mr-2" /> Activate Wallet</> : <><ShieldAlert className="w-3.5 h-3.5 mr-2" /> Freeze Wallet</>}
            </Button>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 p-6 text-white shadow-lg border border-slate-700/50">
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6">
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-indigo-300">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest">Master Wallet Account</span>
                </div>
              ...
              <div className="flex flex-col items-end gap-2">
                <Badge className={cn(
                  ...
                  wallet.status === 'ACTIVE' ? "bg-emerald-500/20 text-emerald-400" : "bg-rose-500/20 text-rose-400"
                )}>
                  {wallet.status}
                </Badge>
                <div className="text-right">
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Wallet Number</p>
                  <p className="text-sm font-mono text-slate-200 tracking-wider">{wallet.walletId}</p>
                </div>
              </div>
            </div>
            ...
            <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              ...
              <div className="space-y-1">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Daily Limit</p>
                <p className="text-xs font-medium text-slate-200">{wallet.dailyLimit.toLocaleString()} THB</p>
              </div>
              <div className="space-y-1">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Monthly Limit</p>
                <p className="text-xs font-medium text-slate-200">{wallet.monthlyLimit.toLocaleString()} THB</p>
              </div>
              <div className="space-y-1">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Security</p>
          ...
          <Card className="border-none shadow-sm ring-1 ring-slate-100 rounded-2xl overflow-hidden bg-white">
            <CardHeader className="p-5 border-b border-slate-100 flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="h-8 text-xs font-medium text-indigo-600 hover:bg-indigo-50">
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100">
                      ...
                      <th className="px-5 py-3 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Details</th>
                      <th className="px-5 py-3 text-[10px] font-semibold text-slate-500 uppercase tracking-wider text-right">Amount</th>
                      ...
                      <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-5 py-3">
                          <div className="space-y-0.5">
                            <p className="text-xs font-semibold text-slate-700">{tx.description || 'System Transaction'}</p>
                        ...
                        <td className="px-5 py-3 text-right">
                          <p className={cn(
                            ...
                            tx.receiverId === wallet.userId ? "text-emerald-600" : "text-slate-700"
                          )}>
                            {tx.receiverId === wallet.userId ? '+' : '-'}{tx.amount.toLocaleString()}
                          </p>
                        </td>
                        <td className="px-5 py-3 text-center">
                          <Badge variant="outline" className={cn(
                            "rounded-md px-2 py-0.5 text-[9px] font-bold uppercase border-none",
                            tx.status === TransactionStatus.SUCCESS ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"
                          )}>
                            {tx.status}
                          </Badge>
                        </td>
        ...
        <div className="space-y-6">
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Badge variant="outline" className="rounded-md px-1.5 py-0 text-[9px] font-semibold border-indigo-100 text-indigo-600 uppercase mt-1">Verified User</Badge>
                    </div>
                  </div>
                  ...
                  <Link href={`/users?search=${user.id}`} className="block pt-2">
                    <Button variant="outline" size="sm" className="w-full h-8 rounded-lg text-xs font-semibold border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600">
          ...
          <Card className="border-none shadow-sm ring-1 ring-slate-100 rounded-2xl bg-white relative overflow-hidden">
            <CardHeader className="p-5 border-b border-slate-100 flex flex-row items-center justify-between">
              ...
              <Badge className="bg-slate-100 text-slate-400 border-none font-black text-[9px] uppercase">Soon</Badge>
            </CardHeader>
            <CardContent className="p-5">
              <Button disabled variant="outline" className="w-full rounded-lg font-semibold text-xs border-slate-100 text-slate-300 bg-slate-50 cursor-not-allowed">
                Manual Adjustment
              </Button>
            </CardContent>
          </Card>

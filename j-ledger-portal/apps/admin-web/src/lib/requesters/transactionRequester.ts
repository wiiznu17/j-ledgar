...
import { Transaction, TransactionDetailsDto, TransferRequest, AdminPaginatedResponse, API_PATHS } from '@repo/dto';

export const transactionRequester = {
  getHistory: async (params?: any) => {
    const query = new URLSearchParams(params).toString();
    return apiClient.get<AdminPaginatedResponse<Transaction>>(`${API_PATHS.ADMIN.TRANSACTIONS.BASE}?${query}`);
  },
